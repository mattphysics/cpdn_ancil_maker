...
#  Date    : 27/03/19
#  Purpose : Functions to replace dump data for a given variable with the one given in file
#
#############################################################################
...
def set_data(fh, fix_hdr, intc, pp_hdrs, pert_data,field,start_idx=-1, n_fields=-1):
    if start_idx == -1:
        start_idx = 0
    if n_fields == -1:
    ...
    for i in range(start_idx, start_idx + n_fields):
        ...
        if field==stash_code:
            ...
            if end_fidx<=len(pert_data):
                ...
                # new_data=[sum(x) for x in zip(old_data*0, pert_data[start_fidx:end_fidx])]
                new_data=[sum(x) for x in zip([0 for d in old_data], pert_data[start_fidx:end_fidx])]
